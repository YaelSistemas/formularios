...
@php
    ...
    $pages = $filasConDatos->chunk(5)->map(fn($chunk) => $chunk->values())->values();

    if ($pages->isEmpty()) {
        $pages = collect([collect()]);
    }

    $alturaBloqueImagenFirma = 160;
@endphp
...
@foreach($pages as $pageIndex => $chunk)
    <div class="sheet">
        ...
        <div style="text-align: center; margin-top: 5px; font-size: 8px; font-weight: bold;">
            Este checklist deberá llenarse cada que se use línea de vida y en caso de no usarse llenarse una vez al mes.
        </div>

        <div style="text-align: center; margin-top: 4px; font-size: 8px; font-weight: bold;">
        ...
        <table class="criteria-table">
            <tr>
                ...
                <td colspan="22" class="top-title">
                    Criterios: ( ✔︎ ) Buen Estado &nbsp;&nbsp; ( X ) Mal Estado &nbsp;&nbsp; ( NA ) No Aplica
                </td>
            </tr>
            ...
            @for ($i = 0; $i < 5; $i++)
                ...
                <tr class="{{ $isEmpty ? 'row-empty' : 'row-filled' }}">
                    ...
                    <td style="font-weight:bold;">
                        {{ $accion === 'El Equipo se Marca como Dañado y es Sacado de Uso' ? 'X' : '' }}
                    </td>
                    
                    <td style="font-weight:bold; font-size:9px;">
                        {{ $accion === 'El Equipo está en Buenas Condiciones' ? '✔︎' : '' }}
                    </td>
        ...
        @if($imagenSrc)
            <div style="margin-top: 6px; width: 100%; position: relative; height: {{ $alturaBloqueImagenFirma }}px;">
                ...
                <div style="position: absolute; right: 250px; top: 70px; text-align: center; width: 180px;">
                    @if(!empty($firmaSrc))
                        <div style="height: 35px; margin-bottom: 2px;">
                            <img src="{{ $firmaSrc }}" style="max-width: 120px; max-height: 50px; object-fit: contain;">
                        </div>
                    @endif
