...
export default function AdminGrupos() {
  ...
  const canViewGrupoHistory = isAdmin;
  ...
  const [historyGrupo, setHistoryGrupo] = useState(null);
  ...
  const openHistory = async (r) => {
    if (!canViewGrupoHistory) {
      setErr("No tienes permiso para ver el historial de grupos.");
      return;
    }
    ...
    try {
      const data = await apiGet(`/admin/grupos/${r.id}/history`);
      setHistoryItems(Array.isArray(data?.history) ? data.history : []);
    } catch (e) {
      setHistoryError(e?.message || "Error cargando historial del grupo");
    } finally {
      setHistoryLoading(false);
    }
  };
  ...
  const S = {
    ...
    table: {
      width: "100%",
      minWidth: canViewGrupoHistory ? 1080 : 920,
                              ...
                              >
                                ...
                                {canDeleteGrupos ? (
                                  <IconBtn
                                    ...
                                    title={
                                      inUse
                                        ? "No se puede eliminar porque está asignado a usuarios"
                                        : "Eliminar"
                                    }
                                  >
          ...
          >
            <div style={S.modalHeader}>
              <div>
                <h3 style={S.modalTitle}>
                  Historial de grupo
                  {historyGrupo?.nombre ? ` - ${historyGrupo.nombre}` : ""}
                </h3>
            ...
            <div style={S.modalBody}>
              {historyLoading ? (
                ...
                <div style={{ color: "#64748b", fontWeight: 700 }}>
                  Este grupo todavía no tiene movimientos registrados.
                </div>
              )}
            </div>
          </div>
