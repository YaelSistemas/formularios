...
export default function SST_POP_TA_04_FO_01_Checklist_de_Sand_Blast({
  form,
  fields,
  answers,
  setVal,
  ...
  const tablaSandBlast = getField("tabla_sand_blast");
  const nombreInspecciona = getField("nombre_inspecciona");
  const firmaInspecciona = getField("firma_inspecciona");
  const nombreSupervisa = getField("nombre_supervisa");
  const firmaSupervisa = getField("firma_supervisa");

  const sandBlastImagenTop =
  "/images/forms/SST_POP_TA_04_FO_01_Checklist_de_Sand_Blast/SandBlast.png";
  ...
  const validateBeforeSubmit = () => {
    ...
    if (!tablaSandBlast) {
      setMsg("No se encontró la tabla de Sand Blast.");
      scrollToTopSafe();
      return false;
    }

    const rows = Array.isArray(answers[tablaSandBlast.id])
      ? answers[tablaSandBlast.id]
      : [];
    ...
    const rowSchema = Array.isArray(tablaSandBlast.row_schema)
      ? tablaSandBlast.row_schema
      : [];
