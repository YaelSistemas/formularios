...
export default function SGI_POP_LG_01_FO_08_Inspeccion_de_Grua_Viajera({
  form,
  fields,
  answers,
  setVal,
  ...
  const buildModalGroups = (rowSchema) => {
    const groups = [];
    const gruaGroupsByNumber = {};
    let capacidadGroup = null;
    ...
    rowSchema.forEach((col) => {
      ...
      const gruaMatch = id.match(/^grua_(\d+)_(estado|observaciones)$/);

      if (id === "capacidad" || id === "observaciones_capacidad") {
        if (!capacidadGroup) {
          capacidadGroup = {
            kind: "section",
            id: "capacidad_grupo",
          ...
          groups.push(capacidadGroup);
        }

        if (id === "capacidad") {
          capacidadGroup.titleField = col;
          capacidadGroup.fields.unshift(col);
          return;
        }

        capacidadGroup.fields.push(col);
        return;
      }

      if (gruaMatch) {
        const groupNumber = gruaMatch[1];

        if (!gruaGroupsByNumber[groupNumber]) {
          gruaGroupsByNumber[groupNumber] = {
            kind: "section",
            id: `grua_${groupNumber}_grupo`,
            titleField: null,
            fields: [],
          };

          groups.push(gruaGroupsByNumber[groupNumber]);
        }

        if (gruaMatch[2] === "estado") {
          gruaGroupsByNumber[groupNumber].titleField = col;
          gruaGroupsByNumber[groupNumber].fields.unshift(col);
          return;
        }

        gruaGroupsByNumber[groupNumber].fields.push(col);
        return;
      }
  ...
  const getModalSectionTitle = (group) => {
    return getCleanFieldLabel(group?.titleField);
  };
  ...
  const getModalFieldDisplayLabel = (col) => {
    ...
    if (id === "observaciones_capacidad") {
      return "Observaciones";
    }

    if (/^grua_\d+_estado$/.test(id)) {
      return "Resultado";
    }

    if (/^grua_\d+_observaciones$/.test(id)) {
      return "Observaciones";
    }
  ...
  const tablaGruaViajera = getField("tabla_grua_viajera");
  const nombreResponsableInspeccion = getField("nombre_responsable_inspeccion");
  const firmaResponsableInspeccion = getField("firma_responsable_inspeccion");
  ...
  const validateBeforeSubmit = () => {
    ...
    const rows = Array.isArray(answers[tablaGruaViajera.id])
      ? answers[tablaGruaViajera.id]
      : [];
    ...
    const rowSchema = Array.isArray(tablaGruaViajera.row_schema)
      ? tablaGruaViajera.row_schema
      : [];
      ...
      >
        ...
        <form onSubmit={handleSubmit}>
          ...
          >
            ...
            {renderOuterRequiredField(nombreResponsableInspeccion)}
            ...
            {renderOuterRequiredField(firmaResponsableInspeccion)}
