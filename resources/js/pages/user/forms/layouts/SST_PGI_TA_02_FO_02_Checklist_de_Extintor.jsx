...
export default function SST_PGI_TA_02_FO_02_Checklist_de_Extintor({
  form,
  fields,
  answers,
  setVal,
  ...
  const showTableModalFieldError = (fieldId, message) => {
    ...
    const rowSchema = Array.isArray(tableModal.field?.row_schema)
      ? tableModal.field.row_schema
      : [];
    const groups = buildModalGroups(rowSchema);
    const targetGroup = groups.find((group) =>
      Array.isArray(group.fields) && group.fields.some((col) => col.id === fieldId)
    );
  ...
  const openTableModal = (field) => {
    ...
    const groups = buildModalGroups(Array.isArray(field?.row_schema) ? field.row_schema : []);
  ...
  const nombreInspector = getField("nombre_responsable_inspeccion");
  const firmaInspector = getField("firma_responsable_inspeccion");
  const indicacionesToggle = getField("indicaciones_llenado");
  const indicacionCriterios = getField("indicacion_criterios");
  const criteriosTitulo = getField("criterios_a_inspeccionar");
  const tablaExtintor = getField("tabla_checklist_extintor");

  const getExtintorDescription = () => "";

  const getSectionTitle = (field) => {
    const id = String(field?.id || "").toLowerCase();

    const titles = {
      componente_extintor: "Componente de Extintor",
    };
  ...
  const validateBeforeSubmit = () => {
    ...
    if (!validateSimpleRequiredField(nombreInspector, "Debes capturar el Nombre del Responsable de Inspección.")) return false;
    if (!validateSimpleRequiredField(firmaInspector, "Debes capturar la Firma del Responsable de Inspección.")) return false;

    if (!tablaExtintor) {
      setMsg("No se encontró la tabla de Checklist de Extintor.");
      scrollToTopSafe();
      return false;
    }

    const rows = Array.isArray(answers[tablaExtintor.id])
      ? answers[tablaExtintor.id]
      : [];

    if (!rows.length) {
      setMsg("Debes agregar al menos una fila en la tabla de Checklist de Extintor.");
      scrollToTopSafe();
      return false;
    }

    const rowSchema = Array.isArray(tablaExtintor.row_schema)
      ? tablaExtintor.row_schema
      : [];
      ...
      >
        ...
        <form onSubmit={handleSubmit}>
          ...
          >
            ...
            {(indicacionesToggle || indicacionCriterios || criteriosTitulo || tablaExtintor) ? (
              <div
                style={{
                  border: "1px solid #e5e7eb",
                  borderRadius: 14,
              ...
              >
                <button
                  type="button"
                  onClick={() => indicacionesToggle ? toggleSection(indicacionesToggle.id) : null}
                  style={{
                    width: "100%",
                    ...
                    textAlign: "left",
                    fontWeight: 800,
                    cursor: indicacionesToggle ? "pointer" : "default",
                    display: "flex",
                    justifyContent: "space-between",
                    alignItems: "center",
                    color: "#0f172a",
                ...
                >
                  <span>{indicacionesToggle?.text || indicacionesToggle?.label || "Checklist de Extintor"}</span>
                  {indicacionesToggle ? <span>{isCollapsed ? "＋" : "－"}</span> : null}
                </button>
                    ...
                    return (
                      <React.Fragment key={group.id}>
                        ...
                        >
                          ...
                          >
                            <span>{getSectionTitle(group.titleField)}</span>
                              ...
                              {group.fields.map((col) => {
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    {(col.description || getExtintorDescription(col.id)) ? (
                                      <div
                                        style={{
                                          marginTop: 8,
                                          marginBottom: 10,
