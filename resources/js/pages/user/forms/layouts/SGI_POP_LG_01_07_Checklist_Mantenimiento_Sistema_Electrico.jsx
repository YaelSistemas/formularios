// CORREGIDO_V3_MODAL_SISTEMA_ELECTRICO: títulos principales agrupan sus subtítulos y campos dentro de una sección/cuadro.
import React, { useEffect, useMemo, useRef, useState } from "react";

export default function SGI_POP_LG_01_07_Checklist_Mantenimiento_Sistema_Electrico({
  form,
  fields,
  answers,
  setVal,
  ...
  const buildModalGroups = (rowSchema) => {
    const groups = [];
    const electricGroupsByKey = {};
    let currentSection = null;

    const pushGroup = (group) => {
      if (currentSection && group.kind !== "section_header" && group.kind !== "image") {
        currentSection.children.push(group);
      } else {
        groups.push(group);
      }
    };
    ...
    rowSchema.forEach((col) => {
      ...
      const electricMatch = id.match(/^electrico_(\d+)(?:_(\d+))?_(estado|observaciones)$/);

      if (electricMatch) {
        const groupKey = electricMatch[2]
          ? `electrico_${electricMatch[1]}_${electricMatch[2]}`
          : `electrico_${electricMatch[1]}`;

        if (!electricGroupsByKey[groupKey]) {
          electricGroupsByKey[groupKey] = {
            kind: "section",
            id: `${groupKey}_grupo`,
            titleField: null,
            fields: [],
          ...
          pushGroup(electricGroupsByKey[groupKey]);
        }

        if (electricMatch[3] === "estado") {
          electricGroupsByKey[groupKey].titleField = col;
          electricGroupsByKey[groupKey].fields.unshift(col);
          return;
        }

        electricGroupsByKey[groupKey].fields.push(col);
        return;
      }

      if (currentSection) {
        currentSection.fields.push(col);
        return;
      }
  ...
  const removeLeadingNumber = (label) => {
    return String(label || "").replace(/^\s*\d+\.\s*/, "");
  };

  const getCleanFieldLabel = (col) => {
    return removeLeadingNumber(col?.text || col?.label || "");
  };

  const sectionTitleById = {
    subestacion_titulo: "1 Subestación",
    banco_capacitores_titulo: "2 Banco de Capacitores",
    transformador_titulo: "3 Transformador de 400 a 200 y 110 volt",
    tableros_contactos_titulo: "4 Tableros de contactos",
    alumbrado_titulo: "5 Alumbrado",
    almacen_frios_titulo: "6 Almacén de fríos",
    oficinas_administrativas_titulo: "7 Oficinas administrativas",
    pararrayos_titulo: "9 Pararrayos",
  };

  const itemTitleByEstadoId = {
    electrico_1_1_estado: "1.1 Interruptores termomagnéticos",
    electrico_1_2_estado: "1.2 Cableado y conexiones",
    electrico_1_3_estado: "1.3 Tablero eléctrico / pastillas",
    electrico_2_1_estado: "2.1 Capacitor",
    electrico_2_2_estado: "2.2 Conexiones",
    electrico_3_1_estado: "3.1 Barras de conexión",
    electrico_3_2_estado: "3.2 Cableado y conexiones",
    electrico_4_1_estado: "4.1 Conectores de ploga a 440 y 220 V",
    electrico_4_2_estado: "4.2 Contactos polarizados a 110 volts",
    electrico_4_3_estado: "4.3 Cableado y conexiones",
    electrico_5_1_estado: "5.1 Lámparas leds almacén",
    electrico_5_2_estado: "5.2 Reflectores almacén",
    electrico_5_3_estado: "5.3 Alumbrado vestidores y WC",
    electrico_5_4_estado: "5.4 Hidroneumático y calentador",
    electrico_6_1_estado: "6.1 Alumbrado",
    electrico_6_2_estado: "6.2 Aterrizaje de racks",
    electrico_7_1_estado: "7.1 Alumbrado y contactos",
    electrico_7_2_estado: "7.2 Red de datos",
    electrico_8_estado: "8 Alumbrado de patios",
    electrico_9_1_estado: "9.1 Sistema de tierras y conexiones",
    electrico_9_2_estado: "9.2 Acoplador y disparador electromagnético",
    electrico_10_estado: "10 Sistema de bombeo cisterna",
    electrico_11_estado: "11 Sistema de video vigilancia",
    electrico_12_estado: "12 Extractores ambientales",
  };

  const getEstadoIdFromObservacionesId = (id) => {
    return String(id || "").replace(/_observaciones$/, "_estado");
  };

  const getModalSectionTitle = (group) => {
    const field = group?.titleField;
    const fieldId = String(field?.id || "");
    const groupId = String(group?.id || "");

    if (group?.kind === "section_header") {
      return sectionTitleById[fieldId] || field?.text || field?.label || "";
    }

    if (fieldId && itemTitleByEstadoId[fieldId]) {
      return itemTitleByEstadoId[fieldId];
    }
    ...
    const groupKey = groupId.replace(/_grupo$/, "_estado");
    return itemTitleByEstadoId[groupKey] || "";
  };

  const getModalFieldDisplayLabel = (col) => {
    const id = String(col?.id || "");

    if (/^electrico_\d+(?:_\d+)?_estado$/.test(id)) {
      return "Resultado";
    }

    if (/^electrico_\d+(?:_\d+)?_observaciones$/.test(id)) {
      return "Observaciones";
    }

    return col?.text || col?.label || "";
  };
  ...
  const descripcion = getField("descripcion");
  const tablaSistemaElectrico = getField("tabla_sistema_electrico");
  ...
  const validateBeforeSubmit = () => {
    ...
    if (!tablaSistemaElectrico) {
      setMsg("No se encontró la tabla de Sistema Eléctrico.");
      scrollToTopSafe();
      return false;
    }

    const rows = Array.isArray(answers[tablaSistemaElectrico.id])
      ? answers[tablaSistemaElectrico.id]
      : [];

    if (!rows.length) {
      setMsg("Debes agregar al menos una fila en la tabla de condiciones eléctricas.");
      scrollToTopSafe();
      return false;
    }

    const rowSchema = Array.isArray(tablaSistemaElectrico.row_schema)
      ? tablaSistemaElectrico.row_schema
      : [];
    ...
    if (!validateSimpleRequiredField(nombreResponsableMantenimiento, "Debes capturar el Nombre del Responsable de Mantenimiento.")) return false;
    if (!validateSimpleRequiredField(firmaResponsableMantenimiento, "Debes capturar la Firma del Responsable de Mantenimiento.")) return false;
      ...
      return (
        <React.Fragment key={group.id}>
          ...
          >
            ...
            >
              {children.length ? (
                children.map((childGroup, childIndex) =>
                  renderModalGroupContent(childGroup, childIndex, children)
                )
              ) : (
                <div
                  style={{
        ...
        <React.Fragment key={group.id}>
          ...
          >
            ...
            >
              ...
              {group.fields.map((col, fieldIndex) => {
      ...
      >
        ...
        <form onSubmit={handleSubmit}>
          ...
          >
            ...
            {renderOuterRequiredField(nombreResponsableMantenimiento)}
            ...
            {renderOuterRequiredField(firmaResponsableMantenimiento)}
              ...
              >
                ...
                {!isCollapsed ? (
                  ...
                  >
                    ...
                    {tablaSistemaElectrico ? (
                      <div style={{ display: "grid", gap: 10 }}>
                        {renderField(tablaSistemaElectrico)}
                      </div>
                    ) : null}
                  </div>
                ) : null}
            ...
            {observacionesGenerales ? (
              <>
                ...
                <div style={fieldBlockStyle}>
                  <label style={{ fontSize: isMobile ? 14 : 14, color: "#0f172a", lineHeight: 1.4 }}>
                    <b>{observacionesGenerales.label}</b>
                  </label>
                  {renderField(observacionesGenerales)}
                </div>
              </>
            ) : null}
          </div>
