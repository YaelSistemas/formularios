...
export default function SST_POP_TA_01_FO_06_Checklist_de_Polipasto_Manual_de_Cadena({
  form,
  fields,
  answers,
  setVal,
  ...
  const tablaPolipasto = getField("tabla_polipasto_manual_cadena");
  ...
  const nombreSupervisorTrabajador = getField("nombre_supervisor_trabajador");
  const firmaSupervisorTrabajador = getField("firma_supervisor_trabajador");

  const polipastoImagenTop =
  "/images/forms/SST_POP_TA_01_FO_06_Checklist_de_Polipasto_Manual_de_Cadena/Polipasto_Manual_Cadena.png";

  const getPolipastoDescription = (id) => {
    const descriptions = {
      polipasto_1_estado:
        "Torcidos, flexionados, con demasiada apertura en la garganta, gastados, agrietados, con muescas, con estrías, oxidado",
  
      polipasto_2_estado:
        "Torcidos, gastados, faltante, oxidado, no funciona",
  
      polipasto_3_estado:
        "Gastados, flojos, oxidados, faltantes",
  
      polipasto_4_estado:
        "Gastados, flojos, oxidados, faltantes",
  
      polipasto_5_estado:
        "Decolorado, gastado, agrietado",
  ...
  const validateBeforeSubmit = () => {
    ...
    if (!tablaPolipasto) {
      setMsg("No se encontró la tabla de Polipasto Manual de Cadena.");
      scrollToTopSafe();
      return false;
    }

    const rows = Array.isArray(answers[tablaPolipasto.id])
      ? answers[tablaPolipasto.id]
      : [];
    ...
    const rowSchema = Array.isArray(tablaPolipasto.row_schema)
      ? tablaPolipasto.row_schema
      : [];
          ...
          >
            ...
            <div style={{ padding: 16, display: "grid", gap: 14 }}>
              ...
              >
                ...
                >
                  <img
                    src={polipastoImagenTop}
                    alt="Polipasto Manual de Cadena"
                    style={{
                      display: "block",
                      width: "100%",
