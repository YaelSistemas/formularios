...
export default function SGI_POP_GT_01_FO_08_Lista_de_Herramientas_Materiales({
  form,
  fields,
  answers,
  setVal,
  ...
  const buildModalGroups = (rowSchema) => {
    return rowSchema
      .filter((col) => col.type !== "static_text")
      .map((col) => ({
        kind: col.type === "fixed_image" ? "image" : "single",
        id: col.id,
        titleField: null,
        fields: [col],
      }));
  ...
  const nombreElabora = getField("nombre_elabora");
  const firmaElabora = getField("firma_elabora");
  const indicacionesToggle = getField("indicaciones_toggle");
  const indicacion1 = getField("indicacion_1");
  const tablaHerramientasMateriales = getField("tabla_herramientas_materiales");
  const nombreRevisa = getField("nombre_revisa");
  const firmaRevisa = getField("firma_revisa");
  ...
  const validateBeforeSubmit = () => {
    ...
    const rows = Array.isArray(answers[tablaHerramientasMateriales.id])
      ? answers[tablaHerramientasMateriales.id]
      : [];

    if (!rows.length) {
      setMsg("Debes agregar al menos una fila en la tabla de herramientas materiales.");
      scrollToTopSafe();
      return false;
    }

    const rowSchema = Array.isArray(tablaHerramientasMateriales.row_schema)
      ? tablaHerramientasMateriales.row_schema
      : [];
          ...
          >
            ...
            >
              {group.titleField ? (
                ...
                >
                  {getCleanFieldLabel(group.titleField)}
                </div>
              ) : null}
      ...
      >
        ...
        <form onSubmit={handleSubmit}>
          ...
          >
            ...
            {renderOuterRequiredField(nombreElabora)}
            ...
            {renderOuterRequiredField(firmaElabora)}
            ...
            {renderOuterRequiredField(nombreRevisa)}
            ...
            {renderOuterRequiredField(firmaRevisa)}
              ...
              >
                ...
                {!isCollapsed ? (
                  ...
                  >
                    ...
                    {tablaHerramientasMateriales ? (
                      <div style={{ display: "grid", gap: 10 }}>
                        {renderField(tablaHerramientasMateriales)}
                      </div>
                    ) : null}
                  </div>
                ) : null}
