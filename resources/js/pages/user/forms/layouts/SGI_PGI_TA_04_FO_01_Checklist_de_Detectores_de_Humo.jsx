...
export default function SGI_PGI_TA_04_FO_01_Checklist_de_Detectores_de_Humo({
  form,
  fields,
  answers,
  setVal,
  ...
  const [modalCollapsedSections, setModalCollapsedSections] = useState({
    criterios_detectores_humo: false,
  });
    ...
    return [
      {
        ...
        id: "criterios_detectores_humo",
        title: "Considerar los siguientes criterios de acuerdo a las condiciones de detectores de humo",
        titleField: {
  ...
  const textoCriteriosDetectoresHumo = getField("texto_criterios_detectores_humo");
  const tablaDetectoresHumo =
    getField("tabla_detectores_humo") ||
    getField("tabla_checklist_detectores_humo") ||
    getField("tabla_inspeccion_detectores_humo");
  ...
  const validateBeforeSubmit = () => {
    ...
    if (!tablaDetectoresHumo) {
      setMsg("No se encontró la tabla de Detectores de Humo.");
      scrollToTopSafe();
      return false;
    }

    const rows = Array.isArray(answers[tablaDetectoresHumo.id])
      ? answers[tablaDetectoresHumo.id]
      : [];

    if (!rows.length) {
      setMsg("Debes agregar al menos una fila en la tabla de detectores de humo.");
      scrollToTopSafe();
      return false;
    }

    const rowSchema = Array.isArray(tablaDetectoresHumo.row_schema)
      ? tablaDetectoresHumo.row_schema
      : [];
                ...
                {!isCollapsed ? (
                  ...
                  >
                    {textoCriteriosDetectoresHumo ? (
                      ...
                      >
                        {textoCriteriosDetectoresHumo.text}
                      </div>
                    ) : null}

                    {tablaDetectoresHumo ? (
                      <div style={{ display: "grid", gap: 10 }}>
                        {renderField(tablaDetectoresHumo)}
                      </div>
                    ) : null}
                  </div>
                ) : null}
