...
export default function SGI_POP_LG_01_FO_09_Checklist_Eslingas_de_Cadenas({
  form,
  fields,
  answers,
  setVal,
  ...
  const saveSignature = () => {
    ...
    if (signatureModal.source === "table") {
      setTableRowVal(field.id, dataUrl);
      if (tableModalErrorFieldId === field.id) clearTableModalError();
    } else {
  ...
  const openTableSignatureModal = (field) => {
    if (readOnly) return;
    if (tableModalErrorFieldId === field?.id) clearTableModalError();
    setSignatureModal({ open: true, field, source: "table" });
  };

  const removeTableSignature = (fieldId) => {
    if (readOnly) return;
    setTableRowVal(fieldId, "");
    if (tableModalErrorFieldId === fieldId) clearTableModalError();
  };
  ...
  const renderTableModalField = (f) => {
    ...
    if (f.type === "signature") {
      const value = tableRowDraft[f.id] || "";
      ...
      return (
        <div style={{ display: "grid", gap: 10 }}>
          {signatureSrc ? (
            <div
              style={{
                border: hasError ? "1px solid #fb923c" : "1px solid #d1d5db",
                borderRadius: 12,
                background: "#fff",
                padding: 10,
                boxShadow: hasError ? "0 0 0 3px rgba(251,146,60,0.12)" : "none",
              }}
            >
              <img
                src={signatureSrc}
            ...
            <div
              style={{
                border: hasError ? "1px solid #fb923c" : "1px dashed #cbd5e1",
                borderRadius: 12,
                background: "#f8fafc",
                padding: isMobile ? 14 : 16,
                textAlign: "center",
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            <button
              ...
              type="button"
              onClick={() => openTableSignatureModal(f)}
              style={{
                borderRadius: 12,
                border: hasError ? "1px solid #fb923c" : "1px solid #c7d2fe",
                background: hasError ? "#fff7ed" : "#eef2ff",
                color: hasError ? "#9a3412" : "#1e40af",
                padding: isMobile ? "10px 14px" : "10px 12px",
                cursor: "pointer",
                fontWeight: 800,
                fontSize: isMobile ? 14 : 14,
            ...
            {value ? (
              <button
                type="button"
                onClick={() => removeTableSignature(f.id)}
                style={{
                  borderRadius: 12,
                  border: "1px solid #fecaca",
                  background: "#fef2f2",
  ...
  const nombreColaboradorInspecciono = getField("nombre_colaborador_inspecciono");
  const firmaColaboradorInspecciono = getField("firma_colaborador_inspecciono");
  const notas = getField("notas");
  const indicacionesToggle = getField("separacion_tabla");
  const indicacionCriterios = getField("indicacion_criterios");
  const criteriosTitulo = getField("criterios_titulo");
  const tablaEslingas = getField("tabla_checklist_eslingas_cadenas");
  const eslingasImageUrl = normalizeAssetUrl("/images/forms/SGI_POP_LG_01_FO_09_Checklist_Eslingas_de_Cadenas/Eslingas.png");

  const getEslingasDescription = () => "";
    ...
    return (
      ...
      >
        <label style={{ fontSize: isMobile ? 14 : 14, color: "#0f172a", lineHeight: 1.4 }}>
          <b>{f.label}</b>{f.required !== false ? <span style={{ color: "crimson" }}> *</span> : null}
        </label>
  ...
  const validateBeforeSubmit = () => {
    ...
    if (!validateSimpleRequiredField(nombreColaboradorInspecciono, "Debes capturar el Nombre del Colaborador que Inspecciono.")) return false;
    if (!validateSimpleRequiredField(firmaColaboradorInspecciono, "Debes capturar la Firma del Colaborador que Inspecciono.")) return false;

    if (!tablaEslingas) {
      setMsg("No se encontró la tabla de Checklist Eslingas de Cadenas.");
      scrollToTopSafe();
      return false;
    }

    const rows = Array.isArray(answers[tablaEslingas.id])
      ? answers[tablaEslingas.id]
      : [];

    if (!rows.length) {
      setMsg("Debes agregar al menos una fila en la tabla de criterios de revisión.");
      scrollToTopSafe();
      return false;
    }

    const rowSchema = Array.isArray(tablaEslingas.row_schema)
      ? tablaEslingas.row_schema
      : [];
  ...
  const renderModalFieldCard = (col) => {
      ...
      return (
        <React.Fragment key={group.id}>
          {group.fields.map((col) => (
            <div key={col.id}>{renderTableModalField(col)}</div>
          ))}
          ...
          >
            ...
            {!isModalSectionCollapsed ? (
              <div style={{ padding: 14, display: "grid", gap: 14 }}>
                {group.fields.map((col) => renderModalFieldCard(col))}
              </div>
            ) : null}
          </div>
      ...
      >
        ...
        <form onSubmit={handleSubmit}>
          ...
          >
            ...
            {renderOuterRequiredField(nombreColaboradorInspecciono)}
            ...
            {renderOuterRequiredField(firmaColaboradorInspecciono)}
            ...
            {(indicacionesToggle || indicacionCriterios || criteriosTitulo || tablaEslingas) ? (
              <div
                style={{
                  border: "1px solid #e5e7eb",
                  borderRadius: 14,
              ...
              >
                <button
                  type="button"
                  onClick={() => toggleSection(indicacionesToggle?.id || "criterios_eslingas")}
                  style={{
                    width: "100%",
                ...
                >
                  <span>
                    {indicacionesToggle?.text || indicacionesToggle?.label || "Criterios de revisión"}
                  </span>
                ...
                {!isCollapsed ? (
                  ...
                  >
                    {indicacionCriterios ? (
                      ...
                      >
                        {indicacionCriterios.text || indicacionCriterios.label}
                      </div>
                    ) : null}
                    ...
                    {criteriosTitulo ? (
                      ...
                      >
                        {criteriosTitulo.text || criteriosTitulo.label}
                      </div>
                    ) : null}

                    {tablaEslingas ? (
                      <div style={{ display: "grid", gap: 10 }}>
                        {renderField(tablaEslingas)}
                      </div>
                    ) : null}
                  </div>
                ) : null}
            ...
            {notas ? (
              <>
                ...
                {renderOuterRequiredField(notas)}
              </>
            ) : null}
          </div>
        ...
        >
          ...
          >
            ...
            <div style={{ padding: 16, display: "grid", gap: 14 }}>
              ...
              >
                <img
                  src={eslingasImageUrl}
                  alt="Eslingas de cadenas"
                  style={{
                    display: "block",
                    width: "100%",
