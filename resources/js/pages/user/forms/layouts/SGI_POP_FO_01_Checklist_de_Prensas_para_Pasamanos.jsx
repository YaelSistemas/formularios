...
export default function SGI_POP_FO_01_Checklist_de_Prensas_para_Pasamanos({
  form,
  fields,
  answers,
  setVal,
  ...
  const showTableModalFieldError = (fieldId, message) => {
    ...
    if (
      String(fieldId || "").startsWith("estado_item_") ||
      String(fieldId || "").startsWith("observaciones_item_")
    ) {
      setModalCollapsedSections((prev) => ({
        ...prev,
        estado_prensa: false,
  ...
  const buildModalGroups = (rowSchema) => {
    ...
    const imageField =
      schema.find((col) => col.type === "fixed_image") ||
      {
        id: "imagen_prensa_pasamanos",
        type: "fixed_image",
        label: "Prensa para pasamanos",
        url: "/images/forms/SGI_POP_FO_01_Checklist_de_Prensas_para_Pasamanos/prensa_pasamanos.jpg",
      };

    if (!imageField.url) {
      imageField.url = "/images/forms/SGI_POP_FO_01_Checklist_de_Prensas_para_Pasamanos/prensa_pasamanos.jpg";
    }

    const accionField = schema.find((col) => col.id === "accion_realizar");
    const criteriosTitle = schema.find((col) => col.id === "subtitulo_criterios");
    const estadoTitle = schema.find((col) => col.id === "subtitulo_estado");

    const criteriosFields = schema.filter((col) =>
      [
        "codigo_identificacion_prensa",
        "tipo_prensa",
        "tipo_voltaje",
    ...
    const estadoFields = schema.filter(
      (col) =>
        String(col.id || "").startsWith("estado_item_") ||
        String(col.id || "").startsWith("observaciones_item_")
    );

    const notasField = schema.find((col) => col.id === "notas");
    ...
    return [
...

      {
        ...
        id: "criterios_prensa",
        title: "Considerar los siguientes criterios de acuerdo a las condiciones de la prensa",
        titleField: criteriosTitle || {
...

      {
        kind: "estado_section",
        id: "estado_prensa",
        title: "Marque según corresponda el estado:",
        titleField: estadoTitle || {
          id: "subtitulo_estado",
          type: "static_text",
  ...
  const nombreResponsableAreaPasamanos = getField("nombre_responsable_area_pasamanos");
  const firmaResponsableAreaPasamanos = getField("firma_responsable_area_pasamanos");
  const indicacionesToggle = getField("indicaciones_toggle");
  const tablaPrensasPasamanos =
    getField("tabla_prensas_pasamanos") ||
    getField("tabla_checklist_prensas_pasamanos") ||
    getField("tabla_prensas_para_pasamanos");
  ...
  const validateBeforeSubmit = () => {
    ...
    if (!validateSimpleRequiredField(nombreInspeccionaMantenimiento, "Debes capturar el Nombre de quien Inspecciona o da Mantenimiento.")) return false;
    if (!validateSimpleRequiredField(firmaInspeccionaMantenimiento, "Debes capturar la Firma de quien Inspecciona o da Mantenimiento.")) return false;
    if (!validateSimpleRequiredField(nombreResponsableAreaPasamanos, "Debes capturar el Nombre del Responsable del Área de Pasamanos.")) return false;
    if (!validateSimpleRequiredField(firmaResponsableAreaPasamanos, "Debes capturar la Firma del Responsable del Área de Pasamanos.")) return false;

    if (!tablaPrensasPasamanos) {
      setMsg("No se encontró la tabla de Prensas para Pasamanos.");
      scrollToTopSafe();
      return false;
    }

    const rows = Array.isArray(answers[tablaPrensasPasamanos.id])
      ? answers[tablaPrensasPasamanos.id]
      : [];

    if (!rows.length) {
      setMsg("Debes agregar al menos una fila en la tabla de prensas para pasamanos.");
      scrollToTopSafe();
      return false;
    }

    const rowSchema = Array.isArray(tablaPrensasPasamanos.row_schema)
      ? tablaPrensasPasamanos.row_schema
      : [];
  ...
  const renderModalGroupContent = (group, index, filteredGroups) => {
    ...
    if (group.kind === "estado_section") {
      const isModalSectionCollapsed = !!modalCollapsedSections[group.id];

      const estadoPairs = [];

      for (let i = 1; i <= 12; i += 1) {
        const estado = group.fields.find((col) => col.id === `estado_item_${i}`);
        const observacion = group.fields.find(
          (col) => col.id === `observaciones_item_${i}`
        );
      ...
      return (
        <React.Fragment key={group.id}>
          ...
          >
            ...
            {!isModalSectionCollapsed ? (
              <div style={{ padding: 14, display: "grid", gap: 14 }}>
                {estadoPairs.map(({ estado, observacion }) => {
                  const hasError = tableModalErrorFieldId === estado.id;

                  return (
                    <div
                      key={estado.id}
                      ref={(el) => {
                        tableFieldWrapRefs.current[estado.id] = el;
                      }}
                      style={{
                        borderRadius: 14,
                        border: hasError
                    ...
                    >
                      <div>
                        ...
                        >
                          {getModalFieldDisplayLabel(estado)}{" "}
                          <span style={{ color: "crimson" }}>*</span>
                        </label>
                      ...
                      {observacion ? (
                        <div
                          ref={(el) => {
                            tableFieldWrapRefs.current[observacion.id] = el;
                          }}
                        >
                          <label
                            style={{
                          ...
                          <div style={{ marginTop: 10 }}>
                            {renderTableModalField(observacion)}
                          </div>
                        </div>
                      ) : null}
                    </div>
      ...
      >
        ...
        <form onSubmit={handleSubmit}>
          ...
          >
            ...
            {renderOuterRequiredField(nombreInspeccionaMantenimiento)}
            ...
            {renderOuterRequiredField(firmaInspeccionaMantenimiento)}
            ...
            {renderOuterRequiredField(nombreResponsableAreaPasamanos)}
            ...
            {renderOuterRequiredField(firmaResponsableAreaPasamanos)}
                ...
                {!isCollapsed ? (
                  ...
                  >
                    {tablaPrensasPasamanos ? (
                      <div style={{ display: "grid", gap: 10 }}>
                        {renderField(tablaPrensasPasamanos)}
                      </div>
                    ) : null}
                  </div>
                ) : null}
