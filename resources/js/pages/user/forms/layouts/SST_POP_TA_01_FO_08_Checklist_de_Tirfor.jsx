...
export default function SST_POP_TA_01_FO_08_Checklist_de_Tirfor({
  form,
  fields,
  answers,
  setVal,
  ...
  const numeroIdentificacion = getField("numero_identificacion");
  const capacidadTirfor = getField("capacidad_tirfor");
  const indicacionesToggle = getField("indicaciones_toggle");
  const indicacion1 = getField("indicacion_1");
  const tablaTirfor = getField("tabla_tirfor");
  const nombreTrabajadorElaboraChecklist = getField("nombre_trabajador_elabora_checklist");
  const firmaTrabajadorElaboraChecklist = getField("firma_trabajador_elabora_checklist");

  const tirforImagenTop =
    "/images/forms/SST_POP_TA_01_FO_08_Checklist_de_Tirfor/Especs_Tirfor.png";
  ...
  const getTirforRow = () => {
    const rows = Array.isArray(answers[tablaTirfor?.id]) ? answers[tablaTirfor.id] : [];
    return rows[0] || {};
  };

  const setTirforVal = (fieldId, value) => {
    if (!tablaTirfor) return;

    const currentRow = getTirforRow();

    setVal(tablaTirfor.id, [
      {
        ...currentRow,
        [fieldId]: value,
      },
  ...
  const renderTirforChecklist = () => {
    if (!tablaTirfor) return null;

    const rowSchema = Array.isArray(tablaTirfor.row_schema)
      ? tablaTirfor.row_schema
      : [];
    ...
    return (
      <div style={{ display: "grid", gap: 14 }}>
        ...
        {rowSchema
          .filter((col) => String(col.id || "").includes("_estado"))
          .map((estadoCol) => {
            const baseId = String(estadoCol.id).replace("_estado", "");
            const obsCol = rowSchema.find(
              (col) => col.id === `${baseId}_observaciones`
            );

            const estadoValue = row[estadoCol.id] ?? "";
            const obsValue = obsCol ? row[obsCol.id] ?? "" : "";
            const hasError = formFieldErrorId === estadoCol.id;

            return (
              <div
                key={estadoCol.id}
                ref={(el) => {
                  formFieldWrapRefs.current[estadoCol.id] = el;
                }}
                style={{
              ...
              >
                ...
                <div style={{ display: "grid", gap: 8 }}>
                  {(Array.isArray(estadoCol.options) ? estadoCol.options : []).map((opt) => (
                    <label
                      key={opt}
                      style={{
                        display: "inline-flex",
                    ...
                    >
                      <input
                        ref={(el) => {
                          if (el && estadoValue === opt) {
                            formFieldRefs.current[estadoCol.id] = el;
                          } else if (el && !formFieldRefs.current[estadoCol.id]) {
                            formFieldRefs.current[estadoCol.id] = el;
                          }
                        }}
                        type="radio"
                        name={estadoCol.id}
                        value={opt}
                        checked={estadoValue === opt}
                        onChange={(e) => {
                          setTirforVal(estadoCol.id, e.target.value);
                          if (formFieldErrorId === estadoCol.id) {
                            clearFormFieldError();
                          }
                        }}
                        disabled={readOnly}
                ...
                {obsCol ? (
                  <div style={{ display: "grid", gap: 8 }}>
                    ...
                    <textarea
                      value={obsValue}
                      onChange={(e) => setTirforVal(obsCol.id, e.target.value)}
                      disabled={readOnly}
                      rows={3}
                      style={{
                        width: "100%",
  ...
  const validateBeforeSubmit = () => {
    ...
    if (!validateSimpleRequiredField(numeroIdentificacion, "Debes capturar el No. de Identificación.")) return false;
    if (!validateSimpleRequiredField(capacidadTirfor, "Debes capturar la Capacidad de Tirfor.")) return false;

    if (!tablaTirfor) {
      setMsg("No se encontró la tabla de Tirfor.");
      scrollToTopSafe();
      return false;
    }
              ...
              >
                ...
                {!isCollapsed ? (
                  ...
                  >
                    ...
                    {tablaTirfor ? renderTirforChecklist() : null}
                  </div>
                ) : null}
              </div>
            ) : null}
