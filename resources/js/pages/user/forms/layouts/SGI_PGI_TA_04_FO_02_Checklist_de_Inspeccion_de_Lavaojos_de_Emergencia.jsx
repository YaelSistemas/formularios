...
export default function SGI_PGI_TA_04_FO_02_Checklist_de_Inspeccion_de_Lavaojos_de_Emergencia({
  form,
  fields,
  answers,
  setVal,
    ...
    return [
      {
        ...
        id: "criterios_lavaojos",
        title: "Considerar los siguientes criterios de acuerdo al estado del lavaojos de emergencia",
        titleField: {
  ...
  const textoCriteriosLavaojos = getField("texto_criterios_lavaojos");
  const tablaLavaojosEmergencia =
    getField("tabla_lavaojos_emergencia") ||
    getField("tabla_checklist_lavaojos_emergencia") ||
    getField("tabla_inspeccion_lavaojos_emergencia");
  ...
  const validateBeforeSubmit = () => {
    ...
    if (!validateSimpleRequiredField(nombreInspector, "Debes capturar el Nombre de Inspector.")) return false;
    if (!validateSimpleRequiredField(firmaInspector, "Debes capturar la Firma del Inspector.")) return false;

    if (!tablaLavaojosEmergencia) {
      setMsg("No se encontró la tabla de Lavaojos de Emergencia.");
      scrollToTopSafe();
      return false;
    }

    const rows = Array.isArray(answers[tablaLavaojosEmergencia.id])
      ? answers[tablaLavaojosEmergencia.id]
      : [];

    if (!rows.length) {
      setMsg("Debes agregar al menos una fila en la tabla de lavaojos de emergencia.");
      scrollToTopSafe();
      return false;
    }

    const rowSchema = Array.isArray(tablaLavaojosEmergencia.row_schema)
      ? tablaLavaojosEmergencia.row_schema
      : [];
          ...
          >
                ...
                {!isCollapsed ? (
                  ...
                  >
                    {textoCriteriosLavaojos ? (
                      ...
                      >
                        {textoCriteriosLavaojos.text}
                      </div>
                    ) : null}

                    {tablaLavaojosEmergencia ? (
                      <div style={{ display: "grid", gap: 10 }}>
                        {renderField(tablaLavaojosEmergencia)}
                      </div>
                    ) : null}
                  </div>
                ) : null}
            ...
            {observacionesGenerales ? (
              <>
                ...
                {renderOuterRequiredField(observacionesGenerales)}
              </>
            ) : null}
          </div>
