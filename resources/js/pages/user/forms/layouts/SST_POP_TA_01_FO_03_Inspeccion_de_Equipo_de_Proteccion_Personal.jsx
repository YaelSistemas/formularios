...
export default function SST_POP_TA_01_FO_03_Inspeccion_de_Equipo_de_Proteccion_Personal({
  form,
  fields,
  answers,
  setVal,
  ...
  const tablaEpp = getField("tabla_inspeccion_epp");

  const getEppDescription = () => "";
  ...
  const validateBeforeSubmit = () => {
    ...
    if (!tablaEpp) {
      setMsg("No se encontró la tabla de Equipo de Protección Personal.");
      scrollToTopSafe();
      return false;
    }

    const rows = Array.isArray(answers[tablaEpp.id])
      ? answers[tablaEpp.id]
      : [];
    ...
    const rowSchema = Array.isArray(tablaEpp.row_schema)
      ? tablaEpp.row_schema
      : [];
      ...
      >
        ...
        <form onSubmit={handleSubmit}>
          ...
          >
            ...
            {(indicacionCriterios || criteriosTitulo || tablaEpp) ? (
              <div
                style={{
                  border: "1px solid #e5e7eb",
                  borderRadius: 14,
              ...
              >
                <button
                  type="button"
                  onClick={() => toggleSection(indicacionesToggle?.id || "separacion_tabla")}
                  style={{
                    width: "100%",
                ...
                >
                  <span>{indicacionesToggle?.text || indicacionesToggle?.label || "Inspección de EPP"}</span>
