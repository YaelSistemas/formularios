...
export default function SGI_POP_GT_01_FO_11_Checklist_de_Inspeccion_de_Estrobos({
  form,
  fields,
  answers,
  setVal,
  ...
  const tablaEstrobos = getField("tabla_estrobos");
  ...
  const validateBeforeSubmit = () => {
    ...
    if (!tablaEstrobos) {
      setMsg("No se encontró la tabla de Inspección de Estrobos.");
      scrollToTopSafe();
      return false;
    }

    const rows = Array.isArray(answers[tablaEstrobos.id])
      ? answers[tablaEstrobos.id]
      : [];

    if (!rows.length) {
      setMsg("Debes agregar al menos una fila en la tabla de inspección de estrobos.");
      scrollToTopSafe();
      return false;
    }

    const rowSchema = Array.isArray(tablaEstrobos.row_schema)
      ? tablaEstrobos.row_schema
      : [];
        ...
        >
          ...
          >
            ...
            <div style={{ padding: 16, display: "grid", gap: 14 }}>
              ...
              >
                <img
                  src="/images/forms/SGI_POP_GT_01_FO_11_Checklist_de_Inspeccion_de_Estrobos/estrobos.png"
                  alt="Guía visual estrobos"
                  style={{
                    display: "block",
                    width: "100%",
              ...
              {modalGroups
                .filter((group) => group.id !== "imagen_estrobo")
