...
export default function SST_POP_TA_04_FO_02_Inspeccion_de_Arnes_de_Seguridad({
  form,
  fields,
  answers,
  setVal,
  ...
  const showTableModalFieldError = (fieldId, message) => {
    ...
    const groupId = getModalGroupIdByFieldId(fieldId);
    if (groupId) {
  ...
  const getInitialModalCollapsedSections = (rowSchema) => {
  ...
  const getModalGroupIdByFieldId = (fieldId) => {
    const schema = Array.isArray(tableModal.field?.row_schema)
      ? tableModal.field.row_schema
      : [];
    const groups = buildModalGroups(schema);
    const found = groups.find((group) =>
      Array.isArray(group.fields) && group.fields.some((field) => field.id === fieldId)
    );
  ...
  const openTableModal = (field) => {
    ...
    setModalCollapsedSections(getInitialModalCollapsedSections(field?.row_schema));
  ...
  const buildModalGroups = (rowSchema) => {
    ...
    rowSchema.forEach((col) => {
      ...
      const belongsToSection =
        id.startsWith("correas_") ||
        id.startsWith("d_ring_") ||
        id.startsWith("hebillas_") ||
        id.startsWith("etiqueta_");
    ...
    return groups.filter((group) => group.kind !== "collapsible_section" || group.fields.length > 0);
  };
  ...
  const tablaLineaVida = getField("tabla_arnes_seguridad");

  const arnesImagenTop =
  "/images/forms/SST_POP_TA_04_FO_02_Inspeccion_de_Arnes_de_Seguridad/ArnesSeguridad.png";
  ...
  const validateBeforeSubmit = () => {
    ...
    if (!tablaLineaVida) {
      setMsg("No se encontró la tabla de arnés de seguridad.");
      scrollToTopSafe();
      return false;
    }

    const rows = Array.isArray(answers[tablaLineaVida.id])
      ? answers[tablaLineaVida.id]
      : [];
    ...
    const rowSchema = Array.isArray(tablaLineaVida.row_schema)
      ? tablaLineaVida.row_schema
      : [];
