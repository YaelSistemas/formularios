...
export default function SST_PGI_TA_02_FO_03_Checklist_de_Botiquines({
  form,
  fields,
  answers,
  setVal,
  ...
  const tablaBotiquines = getField("tabla_checklist_botiquines");

  const getBotiquinesDescription = () => "";

  const getSectionTitle = (field) => {
    const id = String(field?.id || "").toLowerCase();

    const titles = {
      material_seco: "Material seco",
      material_liquido: "Material Líquido",
      material_instrumental: "Material Instrumental",
      material_brigadista: "Material Brigadista",
    };
  ...
  const validateBeforeSubmit = () => {
    ...
    const rows = Array.isArray(answers[tablaBotiquines.id])
      ? answers[tablaBotiquines.id]
      : [];
    ...
    const rowSchema = Array.isArray(tablaBotiquines.row_schema)
      ? tablaBotiquines.row_schema
      : [];
