...
export default function SST_POP_TA_07_FO_01_Inspeccion_de_Compresor({
  ...
  const validateBeforeSubmit = () => {
    ...
    if (
      ...
      String(answers[responsableSeguridad.id] ?? "").trim() === ""
    ) {
      setMsg("Debes capturar el Responsable de Seguridad.");
      scrollToTopSafe();
      return false;
    }

    if (
      !firmaResponsableSeguridad ||
      !answers[firmaResponsableSeguridad.id] ||
      String(answers[firmaResponsableSeguridad.id]).trim() === ""
    ) {
      setMsg("Debes capturar la Firma de Responsable de Seguridad.");
      scrollToTopSafe();
      return false;
    }
        ...
        <form onSubmit={handleSubmit}>
          ...
          >
            ...
            {responsableSeguridad ? (
              <div style={fieldBlockStyle}>
                <label style={{ fontSize: isMobile ? 14 : 14, color: "#0f172a", lineHeight: 1.4 }}>
                  <b>{responsableSeguridad.label}</b> <span style={{ color: "crimson" }}>*</span>
                </label>
                {renderField(responsableSeguridad)}
              </div>
            ) : null}
            ...
            {firmaResponsableSeguridad ? (
              <div style={fieldBlockStyle}>
                <label style={{ fontSize: isMobile ? 14 : 14, color: "#0f172a", lineHeight: 1.4 }}>
                  <b>{firmaResponsableSeguridad.label}</b> <span style={{ color: "crimson" }}>*</span>
                </label>
                {renderField(firmaResponsableSeguridad)}
              </div>
            ) : null}
          </div>
